						...
						<div>
							{
								recentOrders.map((item) => (
									<div className="w-full py-4 px-2 flex items-center gap-3 h-fit">
										<img className="w-24 rounded-xl" src={item.image} alt="recently ordered" />
										<div className="flex flex-col p-0 h-full ">
											<p className="text-base justify-start">{item.Name}</p>
											<p className="text-sm justify-end">{item.description}</p>
											<p className="text-sm  justify-end">{item.date}</p>
										</div>
									</div>
								))
							}
