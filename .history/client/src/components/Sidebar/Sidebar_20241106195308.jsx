...
const Sidebar = ({ sidebarOpen }) => {
  ...
  return (
    <div className={`w-fit  z-50 `}>
      ...
      <div className={`${newSidebar ? 'left-[100px] md:left-[110px]' : '-translate-x-full'} fixed h-screen transition-all duration-300 ease-in-out top-24 shadow-2xl  z-40 rounded-lg  bg-white w-[150px] sm:w-[220px] border-l-[1px]`}>
        {
          mystoredata.map((item) => (
            <Link className="flex gap-4 text-gray-600 items-center pl-5 text-sm px-2 pt-7 hover:text-primary"
              to={`/${item.text}`}
