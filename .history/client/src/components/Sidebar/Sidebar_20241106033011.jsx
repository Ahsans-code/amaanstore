...
const Sidebar = ({ sidebarOpen }) => {
  ...
  return (
    <div className="shadow-2xl">
      ...
      <div className={`${newSidebar ? 'left-[110px]' : '-translate-x-full'} fixed h-screen transition-all duration-300 ease-in-out top-24  z-40 rounded-lg shadow-lg bg-white w-[150px] md:w-[180px] border-l-[1px]`}>
        {
          mystoredata.map((item) => (
            <Link className="flex gap-2 text-gray-600 items-center pl-5 text-xs px-2 pt-7 hover:text-[#4B9E60]"
            to={`/${item.text}`}
